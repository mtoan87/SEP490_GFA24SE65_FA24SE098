...
import './FacilitiesWallet.css';

const FacilitiesWallet = () => {
  const [facilitiesWalletData, setFacilitiesWalletData] = useState([]);
...
useEffect(() => {
    const fetchFacilitiesWalletData = async () => {
        try {
            ...
            setFacilitiesWalletData(Array.isArray(response.data) ? response.data : []);
        } catch (error) {
            console.error(error);
            message.error('Failed to fetch facilities wallet data');
        } finally {
            setLoading(false);
        }
    };
  ...
  return (
    <div className="facilities-wallet-container">
      <h1 className="facilities-wallet-title">Facilities Wallet</h1>
      {loading ? (
        <p className="loading-spinner">Loading...</p>
      ) : (
        <div className="wallet-list">
          {facilitiesWalletData.length > 0 ? (
            <ul className="wallet-items">
              {facilitiesWalletData.map((item) => (
